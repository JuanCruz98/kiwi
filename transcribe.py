...
CHUNK = int(RATE / 10)  # 100ms
    ...
    # connection alive for that long, plus some more to give the API time to fi$
    ...
    def listen_print_loop(self, recognize_stream):
        ...
        for resp in recognize_stream:
            ...
            if not result.is_final:
                ...
                if re.search(r'\b(come va|come stai)\b', transcript, re.I):
                    audio_stream.stop_stream()
                    synthesizer.say("Tutto bene grazie tu?")
                    audio_stream.start_stream()
                ...
                if re.search(r'\b(ore sono|ora &egrave)\b', transcript, re.I):
                    ...
                    audio_stream.start_stream()
                if re.search(r'\b(chi sei|cosa sei)\b', transcript, re.I):
                    audio_stream.stop_stream()
                    synthesizer.say('Sono kiwi, il tuo assistente personale.')
                    audio_stream.start_stream()
 
                num_chars_printed = 0
        print ("stop listen_print")
